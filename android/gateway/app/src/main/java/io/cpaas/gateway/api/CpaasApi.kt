...
class CpaasApi(private val baseUrl: String) {
  ...
  fun pair(code: String, name: String, model: String, manufacturer: String): JSONObject {
    val body = JSONObject()
      .put("code", code)
      .put("name", name)
      .put("model", model)
  ...
  fun heartbeat(
    ...
    if (!simOperator.isNullOrBlank()) payload.put("simOperator", simOperator)
    val req = Request.Builder()
  ...
  fun status(deviceToken: String): JSONObject {
    val req = Request.Builder()
      .url("$root/v1/device/status")
      .addHeader("X-Device-Token", deviceToken)
      .get()
      .build()
    return execute(req)
  ...
  private fun execute(req: Request): JSONObject {
    client.newCall(req).execute().use { res ->
      ...
      if (!res.isSuccessful) {
        ...
        }.getOrNull().orEmpty().ifBlank { text }
        throw IllegalStateException("HTTP ${res.code}: ${message.ifBlank { "request failed" }}")
      }
